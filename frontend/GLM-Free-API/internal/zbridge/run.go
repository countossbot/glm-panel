...
func NewHandler() http.Handler {
    ...
    mux.HandleFunc("/v1/responses", authMiddleware(responsesHandler))
...
`, config.Server.Port, config.Server.Port, config.Server.Port, tokenPadded)
    ...
    // responses finish (STREAM_DRAIN_TIMEOUT, default 180s — long enough for
    // a thinking+answer stream to run to completion, so panel-driven restarts
    // no longer truncate live chats; srv.Shutdown returns the moment the last
    // handler finishes, so with no active streams the drain is instant), then
    // deletes every still-pooled chat session on Z.AI so nothing is left
    // behind on the account. A second CTRL+C force-exits immediately
    // (default handling is re-armed).
    ...
    case <-ctx.Done():
        ...
        drainCtx, cancel := context.WithTimeout(context.Background(), time.Duration(config.StreamDrainTimeout)*time.Second)
