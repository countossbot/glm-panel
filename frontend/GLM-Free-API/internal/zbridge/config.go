...
var accessKey = os.Getenv("ALIYUN_CAPTCHA_ACCESS_KEY")
var secretKey = os.Getenv("ALIYUN_CAPTCHA_SECRET")
...
// captchaSecretKey signs Aliyun captcha API calls. Compile-time default is a
// placeholder; the real value is injected at runtime via ALIYUN_CAPTCHA_SECRET
// (see loadConfig + aliyun.env).
var captchaSecretKey = "[REDACTED:aliyun_secret_key]"
...
type Config struct {
    ...
    // StreamDrainTimeout bounds, in seconds, how long graceful shutdown
    // waits for in-flight streaming responses to finish before closing
    // their connections (STREAM_DRAIN_TIMEOUT, default 180). Panel-driven
    // restarts (config save / restart button) previously cut active streams
    // after only 10s — clients saw "Response truncated — stream ended
    // before completion". srv.Shutdown returns as soon as the last handler
    // finishes, so with no live streams the drain is instant.
    StreamDrainTimeout int
}
...
func loadConfig() *Config {
    ...
    if s := os.Getenv("ALIYUN_CAPTCHA_SECRET"); s != "" {
        captchaSecretKey = s
    }
    ...
    if h := os.Getenv("STREAM_DRAIN_TIMEOUT"); h != "" {
        if n, err := strconv.Atoi(h); err == nil && n > 0 {
            c.StreamDrainTimeout = n
        }
    }
