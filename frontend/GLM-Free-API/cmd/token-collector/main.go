...
var (
	...
	topupFlag         = flag.Bool("topup", false, "append tokens to the existing tokens.sqlite without wiping it")
)
...
func run(tokenCount, batchCount, parallelWorkers int, headed bool, topup bool) error {
	...
	// Database path. Normal collection starts fresh; --topup preserves the
	// live database so zai-api can continue using the same WAL-backed file.
...
func main() {
	...
	if !useTUI {
		...
		if err := run(tokenCount, batchCount, parallelWorkers, *headedFlag, *topupFlag); err != nil {
	...
	go func() {
		err := run(tokenCount, batchCount, parallelWorkers, *headedFlag, *topupFlag)
		if err == nil {
