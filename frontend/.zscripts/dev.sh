...
start_glm_infra() {
        log_step_start "Starting GLM bridge infra (bridge daemon + token watchdog)"

        # ---- Bridge daemon: honor panel config (GLM-Free-API/admin-config.json) ----
        local cfg="$PROJECT_DIR/GLM-Free-API/admin-config.json"
        local token=""
        if [ -f "$cfg" ] && command -v python3 >/dev/null 2>&1; then
                token=$(python3 -c '
import json, sys
try:
    print((json.load(open(sys.argv[1])).get("zaiToken") or "").strip())
except Exception:
    print(""
' "$cfg" 2>/dev/null || echo "")
        fi
        if [ -n "$token" ]; then
                export ZAI_TOKEN="$token"
        fi
        local flags=""
        if [ -f "$cfg" ] && grep -q '"agentMode"[[:space:]]*:[[:space:]]*true' "$cfg" 2>/dev/null; then
                flags="--agent-mode"
        fi

        local bridge_running=false
        if [ -f "$PROJECT_DIR/GLM-Free-API/server.pid" ]; then
                local bpid
                bpid=$(cat "$PROJECT_DIR/GLM-Free-API/server.pid" 2>/dev/null || echo "")
                if [ -n "$bpid" ] && kill -0 "$bpid" 2>/dev/null; then
                        bridge_running=true
                fi
        fi
        if [ "$bridge_running" = true ]; then
                echo "[infra] zai-api already running"
        else
                echo "[infra] starting zai-api $flags (token: $([ -n "$token" ] && echo set || echo guest))"
                bash "$PROJECT_DIR/scripts/start_zai_api.sh" $flags \
                        || echo "[infra] WARNING: zai-api start failed — start it from the admin panel"
        fi

        # ---- Captcha token auto-refill watchdog ----
        if pgrep -f "scripts/token_watchdog.sh" >/dev/null 2>&1; then
                echo "[infra] token watchdog already running"
        else
                echo "[infra] starting token watchdog"
                bash "$PROJECT_DIR/scripts/token_watchdog_ctl.sh" start \
                        || echo "[infra] WARNING: token watchdog start failed"
        fi

        log_step_end "Starting GLM bridge infra"
}
